...
这个错误是因为你的 `protobuf` Python 包版本过新（≥4.21.0），而项目中的 `_pb2.py` 文件是由旧版 `protoc` 生成的，两者不兼容。
...
# 检查当前版本
protoc --version
# 如果版本过低，从 https://github.com/protocolbuffers/protobuf/releases 下载安装
```
...
- 重新生成后，错误即可消失，且能使用 protobuf 最新特性。
...
- protobuf 4.x 改变了描述符的创建方式，旧版生成的 `_pb2.py` 中使用了被移除的内部 API。
- 如果你无法重新生成 proto 文件（例如来自第三方库），降级是更快的临时方案。
