...
*   **硬件配置**：建议为 etcd 节点配备高性能 SSD 磁盘，这对性能至关重要[reference:8]。例如，一个 3 节点集群，每个节点推荐配置为 2核4G[reference:9]。

#### **2. 集群部署示例**
假设我们部署一个 3 节点的集群，节点信息如下：
| 节点名 | IP地址 | 客户端端口 (2379) | 集群通信端口 (2380) |
| :--- | :--- | :--- | :--- |
...
```
*注意：`node2` 和 `node3` 的启动命令需相应修改 `name`、`data-dir` 和各自的 IP 地址[reference:10]。*
