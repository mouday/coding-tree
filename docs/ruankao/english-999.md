# 英文词汇

Virtual Reality 虚拟现实 

Cloud computing 云计算 

Big data 大数据

Internet+ 互联网+

Artificial intelligence 人工智能 

Integrated circuit 集成电路 

Parallel computing 并行计算

Create scope 创建范围

Project stakeholder management 项目干系人管理

Proiect scope management 项目范围管理

Proiect cost management 项目成本管理


## 第19章 配置与变更管理

配置库 Configuration Library

变更控制委员会 Change Control Board, CCB

配置管理数据库 Configuration Management Databases, CMDB


Project Initiiation	项目启动

Scope Definition	范围定义

Resource Allocation	资源分配

Risk Assessment	风险评估

Stakeholder Engagement	干系人参与

Project Planning	项目规划

Schedule Development	进度计划制定

Budget Estimation	预算估算

Quality Planning	质量规划

Team Formation	团队组建

Project Execution	项目执行

Task Assignment	任务分配

Progress Tracking	进度跟踪

Cost Control	成本控制

Change Management	变更管理

Communication Plan	沟通计划

Risk Mitigation	风险缓解

Quality Assurance	质量保证

Team Motivation	团队激励

Project Monitoring	项目监控

Scope Verification	范围验证

Earned Value Analysis	挣值分析

Performance Reporting	绩效报告

Issue Resolution	问题解决

Contract Management	合同管理

Procurement Planning	采购规划

Supplier Selection	供应商选择

Integration Management	整合管理

Scope Creep Control	范围蔓延控制

Time Management	时间管理

Cost Optimization	成本优化

Quality Improvement	质量改进

Communication Channels	沟通渠道

Stakeholder Analysis	干系人分析

Project Charter	项目章程

Work Breakdown Structure (WBS)	工作分解结构

Project Management Plan (PMP)	项目管理计划

Risk Register	风险登记册

Change Request Form	变更请求表

Project Status Report	项目状态报告

Project Closure Report	项目关闭报告

Lessons Learned Document	经验教训文档

Project Review Meeting	项目回顾会议

Critical Path Analysis (CPA)	关键路径分析

Monte Carlo Simulation	蒙特卡洛模拟

Agile Project Management	敏捷项目管理

Scrum Framework	Scrum框架

Kanban Board	看板

Sprint Planning	冲刺计划

Daily Stand-up Meeting	每日站会

Sprint Review	冲刺评审

Sprint Retrospective	冲刺回顾

Product Backlog	产品待办事项列表

User Story Mapping	用户故事地图

Continuous Integration	持续集成

Continuous Deployment	持续部署

Test-Driven Development (TDD)	测试驱动开发

Pair Programming	对编程

Refactoring	重构

Code Review	代码审查

Project Portfolio Management	项目组合管理

Program Management	项目群管理

Portfolio Optimization	组合优化

Benefits Realization	收益实现

Roadmap Planning	路线图规划

Strategic Alignment	战略对齐

Value Stream Mapping	价值流映射

Lean Management	精益管理

Six Sigma	六西格玛

Total Quality Management (TQM)	全面质量管理

Root Cause Analysis (RCA)	根本原因分析

Failure Mode and Effects Analysis (FMEA)	故障模式与影响分析

Design for Manufacturability (DFM)	可制造性设计

Design for Assembly (DFA)	可装配性设计

Just-In-Time (JIT)	准时制

Kanban Flow	看板流程

Pull System	拉动系统

Push System	推动系统

Inventory Management	库存管理

Supply Chain Optimization	供应链优化

Vendor Management	供应商管理

Customer Relationship Management (CRM)	客户关系管理

Business Intelligence (BI)	商业智能

Data Analytics	数据分析

Big Data	大数据

Cloud Computing	云计算

Virtualization	虚拟化

Cybersecurity	网络安全

Information Security	信息安全

Data Privacy	数据隐私

Compliance Management	合规管理

Regulatory Compliance	法规合规

Ethical Hacking	道德黑客

Penetration Testing	渗透测试

Disaster Recovery Plan	灾难恢复计划

Business Continuity Plan	业务连续性计划

Change Management Plan	变更管理计划

Knowledge Management	知识管理

Intellectual Property (IP)	知识产权

Patent Application	专利申请

Trademark Registration	商标注册

Copyright Protection	版权保护

License Agreement	许可协议

Non-Disclosure Agreement (NDA)	保密协议

Conflict Resolution	冲突解决

Negotiation Skills	谈判技巧

Mediation Process	调解过程

Alternative Dispute Resolution (ADR)	替代性争议解决

Contract Breach	合同违约

Arbitration	仲裁

Litigation	诉讼

Legal Compliance	法律合规

Auditing	审计

Internal Controls	内部控制

Financial Reporting	财务报告

Cost Accounting	成本会计

Management Accounting	管理会计

Financial Analysis	财务分析

Capital Budgeting	资本预算

Return on Investment (ROI) Analysis	投资回报率分析


Create scope 创建范围
Project stakeholder management 项目干系人管理
Proiect scope management 项目范围管理
Proiect cost management 项目成本管理



Confidentiality 机密性

Consistency 一致性

Integrity 完整性

Availability 可用性


Equipment security 设备安全

Network security 网络安全

Information security 信息安全

Content security 内容安全

feasibility 可行性
